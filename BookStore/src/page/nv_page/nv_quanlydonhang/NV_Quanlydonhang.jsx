...
export default function NV_Quanlydonhang() {
  ...
  const fetchDonHang = async () => {
    try {
      ...
      console.error(t("donhang.order_management.error"));
  ...
  const handleViewDetail = async (record) => {
    try {
      ...
      if (res.code === 0 && res.result) {
        ...
        message.error(t("donhang.order_management.error"));
      }
    } catch (error) {
      console.error(error);
      message.error(t("donhang.order_management.error"));
  ...
  const columns = [
    {
      title: t("donhang.order_management.columns.order_id"),
      dataIndex: "maDonHang",
      key: "maDonHang",
      render: (text) => <span className="font-semibold">{text}</span>,
    },
    {
      title: t("donhang.order_management.columns.customer"),
      dataIndex: "tenNguoiNhan",
      key: "khachHang",
    },
    {
      title: t("donhang.order_management.columns.order_date"),
      dataIndex: "ngayDat",
      key: "ngayDat",
      render: (text) => new Date(text).toLocaleDateString("vi-VN"),
    },
    {
      title: t("donhang.order_management.columns.status"),
      dataIndex: "trangThai",
      key: "trangThai",
      render: (status) => getStatusTag(status),
    },
    {
      title: t("donhang.order_management.columns.amount"),
      key: "tongTien",
      render: (_, record) => {
    ...
    {
      title: t("donhang.order_management.columns.action"),
      key: "action",
      render: (_, record) => (
        <Button
          type="text"
        ...
        >
          {t("donhang.order_management.view_btn")}
        </Button>
      ),
    },
  ];
